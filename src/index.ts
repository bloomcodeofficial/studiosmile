...
$('.portfolio_item').mouseenter(function () {
  ...
  const title = $(this).find('.portfolio_content-title').eq(0).find('div').eq(0).text();
...
$(document).on('input', '#player-time', function () {
  const val = $(this).val();
  console.log(val, 'x');
  const { duration } = $('#player')[0];
  const currentTime = duration * (val / 100);
  $('#player')[0].currentTime = currentTime;

  const currentItem = $('.portfolio_item.currently-hovering');
...
audio.addEventListener('timeupdate', (event) => {
  ...
  const { duration } = $('#player')[0];
  const one_prec = 100 / duration;
  const currentTime_in_perc = one_prec * currentTime;
...
$('body').on('click', '#volume-icon', function (e) {
  const currentVolume = $('#volume').val();
...
let visualiser_loaded = false;

window.AudioContext = window.AudioContext || window.webkitAudioContext || window.mozAudioContext;

const start_visualiser = function () {
  ...
  const canvas = document.getElementById('visualiser-canvas'),
    ...
    meterNum = 600 / (10 + 2),
    capYPositionArray = [];
  (ctx = canvas.getContext('2d')), (line_color = '#FACC15');
  ...
  function renderFrame() {
    ...
    if (!myAudio.paused) {
      for (let i = 0; i < meterNum; i++) {
        ...
        const pos_y = cheight - value + capHeight;
